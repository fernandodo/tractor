...
## 移动
- [ ] `h j k l` → 左 / 下 / 上 / 右 **记住`j`是向下**, `j` 向左就是左
	- [ ] 用 `10j` 可以一次向下 10 行。  
- [ ] `0` / `^` / `$` → 行首 / 行首非空白 / 行尾  
  **例子**：在行中间，按 `$` 跳到行尾。  
...
- `cw` → 修改到下一个词首  
  - `int value;` 光标在 `v` 上，按 `cw` 输入 `count` → 结果 `int count;`  
- `c$` → 修改到行尾  
  - `printf("Hello world");` 光标在 `w` 上，按 `c$` 输入 `VSCode");` → 结果 `printf("Hello VSCode");`  
- `cc` → 修改整行（保持缩进）  
  - `    int x = 0;` 按 `cc` 输入 `float y = 1.5;` → 结果 `    float y = 1.5;`  
- `ci"` → 修改引号内内容  
  - `char *msg = "old text";` 光标在引号内，按 `ci"` 输入 `new text` → 结果 `char *msg = "new text";`  
- `ci(` → 修改括号内内容  
  - `printf("Hello");` 光标在 `Hello` 内，按 `ci(` 输入 `"World"` → 结果 `printf("World");`  
- `ci_` → 修改下划线之间的内容  
  - `fv_abc_def` 光标在 `abc` 上，按 `ci_` 输入 `xyz` → 结果 `fv_xyz_def`  
- `ct_` → 修改到下划线为止  
  - `fv_abc_def` 光标在开头，按 `ct_` 输入 `new` → 结果 `new_abc_def`  
- `cT_` → 修改到上个下划线为止  
  - `fv_abc_def` 光标在 `def` 上，按 `cT_` 输入 `xyz` → 结果 `fv_abc_xyz`  
